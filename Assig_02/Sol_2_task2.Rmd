...
```
```{r}
# Interpretation: significance, sign, conclusion about hypothesis
```
...
## 6)
?????????????????????????
2 new hypotheses have to be formulated
?????????????????????????
```{r}
head(lawyerattr)
```
