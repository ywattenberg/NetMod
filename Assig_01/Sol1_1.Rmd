...
library(sna)
library(network)
obsMat <- as.matrix(read.csv("matrix.csv", header=F))
...
## 5)
### 5.1)
The hypothesis of the first test is that the dyadic reciprocity of the graph is higher/(lower) than expected by chance. The test is conditioned on the edges that means on the network density or number of edges in the network or $U|p$ where p denotes the network density. 
